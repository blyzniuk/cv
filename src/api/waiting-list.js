...
export default async function handler(req, res) {
  const { GOOGLE_FORM_ID } = process.env;
  ...
  try {
    ...
    const gfRes = await fetch(
      `https://docs.google.com/forms/u/1/d/e/${GOOGLE_FORM_ID}/formResponse`,
      {
        method: "POST",
        body: form,
      }
    ...
    res
      .status(500)
      .json({ message: `Google forms request failed with ${gfRes.status}` });
    return;
  } catch (e) {
    console.log(e);
